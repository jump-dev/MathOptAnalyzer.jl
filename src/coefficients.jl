...
abstract type AbstractNumericalIssue end
...
function _summarize(io::IO, ::Type{VariableNotInConstraints})
    return print(io, "# VariableNotInConstraints")
end

function _summarize(io::IO, ::Type{EmptyConstraint})
    return print(io, "# EmptyConstraint")
end

function _summarize(io::IO, ::Type{VariableBoundAsConstraint})
    return print(io, "# VariableBoundAsConstraint")
end

function _summarize(io::IO, ::Type{DenseConstraint})
    return print(io, "# DenseConstraint")
end

function _summarize(io::IO, ::Type{SmallMatrixCoefficient})
    return print(io, "# SmallMatrixCoefficient")
end

function _summarize(io::IO, ::Type{LargeMatrixCoefficient})
    return print(io, "# LargeMatrixCoefficient")
end

function _summarize(io::IO, ::Type{SmallBoundCoefficient})
    return print(io, "# SmallBoundCoefficient")
end

function _summarize(io::IO, ::Type{LargeBoundCoefficient})
    return print(io, "# LargeBoundCoefficient")
end

function _summarize(io::IO, ::Type{SmallRHSCoefficient})
    return print(io, "# SmallRHSCoefficient")
end

function _summarize(io::IO, ::Type{LargeRHSCoefficient})
    return print(io, "# LargeRHSCoefficient")
end

function _summarize(io::IO, ::Type{SmallObjectiveCoefficient})
    return print(io, "# SmallObjectiveCoefficient")
end

function _summarize(io::IO, ::Type{LargeObjectiveCoefficient})
    return print(io, "# LargeObjectiveCoefficient")
end

function _summarize(io::IO, ::Type{SmallObjectiveQuadraticCoefficient})
    return print(io, "# SmallObjectiveQuadraticCoefficient")
end

function _summarize(io::IO, ::Type{LargeObjectiveQuadraticCoefficient})
    return print(io, "# LargeObjectiveQuadraticCoefficient")
end

function _summarize(io::IO, ::Type{NonconvexQuadraticConstraint})
    return print(io, "# NonconvexQuadraticConstraint")
end

function _summarize(io::IO, ::Type{SmallMatrixQuadraticCoefficient})
    return print(io, "# SmallMatrixQuadraticCoefficient")
end

function _summarize(io::IO, ::Type{LargeMatrixQuadraticCoefficient})
    return print(io, "# LargeMatrixQuadraticCoefficient")
end

function _verbose_summarize(io::IO, ::Type{VariableNotInConstraints})
    return print(
        io,
        """
...
function _verbose_summarize(io::IO, ::Type{EmptyConstraint})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{VariableBoundAsConstraint})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{DenseConstraint})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{SmallMatrixCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{LargeMatrixCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{SmallBoundCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{LargeBoundCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{SmallRHSCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{LargeRHSCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{SmallObjectiveCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{LargeObjectiveCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{SmallObjectiveQuadraticCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{LargeObjectiveQuadraticCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{NonconvexQuadraticConstraint})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{SmallMatrixQuadraticCoefficient})
    return print(
        io,
        """
        ## What
...
function _verbose_summarize(io::IO, ::Type{LargeMatrixQuadraticCoefficient})
    return print(
        io,
        """
        ## What
...
function _summarize(io::IO, issue::VariableNotInConstraints)
    return print(io, _name(issue.ref))
end

function _summarize(io::IO, issue::EmptyConstraint)
    return print(io, _name(issue.ref))
end

function _summarize(io::IO, issue::VariableBoundAsConstraint)
    return print(io, _name(issue.ref))
end

function _summarize(io::IO, issue::DenseConstraint)
...
function _summarize(io::IO, issue::LargeBoundCoefficient)
    return print(io, _name(issue.variable), " : ", issue.coefficient)
end

function _summarize(io::IO, issue::SmallRHSCoefficient)
    return print(io, _name(issue.ref), " : ", issue.coefficient)
end

function _summarize(io::IO, issue::LargeRHSCoefficient)
    return print(io, _name(issue.ref), " : ", issue.coefficient)
end

function _summarize(io::IO, issue::SmallObjectiveCoefficient)
    return print(io, _name(issue.variable), " : ", issue.coefficient)
end

function _summarize(io::IO, issue::LargeObjectiveCoefficient)
    return print(io, _name(issue.variable), " : ", issue.coefficient)
end
...
function _verbose_summarize(io::IO, issue::EmptyConstraint)
    return print(io, "Constraint: ", _name(issue.ref))
end

function _verbose_summarize(io::IO, issue::VariableBoundAsConstraint)
    return print(io, "Constraint: ", _name(issue.ref))
end
...
function list_of_issues(data::Data, ::Type{VariableNotInConstraints})
    return data.variables_not_in_constraints
end

function list_of_issues(data::Data, ::Type{EmptyConstraint})
    return data.empty_rows
end

function list_of_issues(data::Data, ::Type{VariableBoundAsConstraint})
    return data.bound_rows
end

function list_of_issues(data::Data, ::Type{DenseConstraint})
    return data.dense_rows
end

function list_of_issues(data::Data, ::Type{SmallMatrixCoefficient})
    return data.matrix_small
end

function list_of_issues(data::Data, ::Type{LargeMatrixCoefficient})
    return data.matrix_large
end

function list_of_issues(data::Data, ::Type{SmallBoundCoefficient})
    return data.bounds_small
end

function list_of_issues(data::Data, ::Type{LargeBoundCoefficient})
    return data.bounds_large
end

function list_of_issues(data::Data, ::Type{SmallRHSCoefficient})
    return data.rhs_small
end

function list_of_issues(data::Data, ::Type{LargeRHSCoefficient})
    return data.rhs_large
end

function list_of_issues(data::Data, ::Type{SmallObjectiveCoefficient})
    return data.objective_small
end

function list_of_issues(data::Data, ::Type{LargeObjectiveCoefficient})
    return data.objective_large
end

function list_of_issues(data::Data, ::Type{SmallObjectiveQuadraticCoefficient})
    return data.objective_quadratic_small
end

function list_of_issues(data::Data, ::Type{LargeObjectiveQuadraticCoefficient})
    return data.objective_quadratic_large
end

function list_of_issues(data::Data, ::Type{NonconvexQuadraticConstraint})
    return data.nonconvex_rows
end

function list_of_issues(data::Data, ::Type{SmallMatrixQuadraticCoefficient})
    return data.matrix_quadratic_small
end

function list_of_issues(data::Data, ::Type{LargeMatrixQuadraticCoefficient})
    return data.matrix_quadratic_large
end

function list_of_issue_types(data::Data)
    ret = Type[]
    for type in (
        VariableNotInConstraints,
        EmptyConstraint,
        ...
        if !isempty(list_of_issues(data, type))
            push!(ret, type)
        end
    end
    return ret
...
function summarize(io::IO, data::Data; verbose = true, max_issues = typemax(Int), configurations = true, dimensions = true, ranges = true)
    ...
    for issue_type in list_of_issue_types(data)
        issues = list_of_issues(data, issue_type)
        print(io, "\n\n")
        summarize(io, issues, verbose = verbose, max_issues = max_issues)
    end
    return
end

function summarize(data::Data; kwargs...)
    return summarize(stdout, data; kwargs...)
end

function Base.show(io::IO, data::Data)
    n = sum(
        length(list_of_issues(data, T)) for T in list_of_issue_types(data);
        init = 0,
    )
